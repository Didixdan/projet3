...
int main(int argc, char * argv[])
    ...
    int sockUDP,fdCarte;
    int ch=0;
    int bouton;/* pour les différents clicks à la souris*/
    int numLemmingARetirer=0;
    ...
    if(strcmp(mode,"MASTER")==0) {
        ...
        if((sockTCPSlave = accept(sockTCP, NULL, NULL)) == -1) {
            ...
            /* on set le timer */
            itsMoveLemming.it_value.tv_sec = 3;
            itsMoveLemming.it_value.tv_nsec = 0;
            itsMoveLemming.it_interval.tv_sec = 3;
                    ...
                    switch(ch){
                        case KEY_MOUSE:
                            if(souris_getpos(&sourisX, &sourisY, &bouton) == OK){
                                ...
                                /*retrait d'un lemming à l'aide du click droit*/
                                if(bouton & BUTTON3_CLICKED){
                                    modeJeu=1;
                                    if(carte.cases[(sourisY*carte.largeur)+sourisX]!='2') continue;
                                    if((sourisX >= 1) && (sourisX <= limitX) && (sourisY >= 1) && (sourisY <= limitY)){
                                        
                                        etatUnLemming.valeur = VAL_LEMMING_RETIRE;
                                        etatUnLemming.posY = 0;
                                        etatUnLemming.posX = 0;
                                        ...
                                        /* on modifie le tableau */
                                        lesLemmings[numLemmingARetirer-1]=TYPE_LEMMING_NON_PLACER;
                                        /* simulation du clique pour ajouter l'affichage de l'indice numLemmingAPlacer */
                                        cliqueNumLemming(legende,&numLemmingARetirer,lesLemmings,BOUTON_PLUS_LEG);
